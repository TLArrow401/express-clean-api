...
export default class UserRepository {
  ...
  async getAll(): Promise<UserModel.UserResponseDTO[]> {
    try {
      const rows = await this.db.query<UserModel.UserRow>(
        "SELECT id, username, email, status, last_login_at, created_at  FROM security.users WHERE delete_at IS NULL",
      );
      // Conversion de datos db a code a response
      return rows.map(this.mapToResponse);
    } catch (error) {
      logger.error({ error: error },"Error fetching users");
  ...
  async getById(id: number): Promise<UserModel.UserResponseDTO | null> {
    try {
      const [user] = await this.db.query<UserModel.UserRow>(
        "SELECT id, username, email, status, last_login_at, created_at FROM security.users WHERE id = $1 AND delete_at IS NULL",
        [id],
      );

      return user ? this.mapToResponse(user) : null;
    } catch (error) {
      logger.error({ error: error },"Error fetching user by id");
      throw new AppError("Database error fetching user by id", 500);
    }
  }
  // Metodo de creacion
  async create(
    data: UserModel.CreateUserDTO,
  ): Promise<UserModel.UserResponseDTO> {
    try {
      const rows = await this.db.query<UserModel.UserRow>(
        `INSERT INTO security.users (username, email, password, status, created_at) VALUES ($1, $2, $3, 'active', NOW()) RETURNING id, username, email, status, last_login_at, created_at`,
      ...
      logger.error({ error: error },"Error creating user");
  ...
  ): Promise<UserModel.UserResponseDTO | null> {
    try {
      ...
      const entries = Object.entries(data).filter(
        ([key, value]) =>
          allowedFields.includes(key as keyof UserModel.UpdateUserDTO) &&
          value !== undefined,
      );
      ...
      const rows = await this.db.query<UserModel.UserRow>(
        `UPDATE security.users SET ${fields.join(", ")} WHERE id = $${values.length} AND delete_at IS NULL RETURNING id, username, email, status, last_login_at, created_at`,
        values,
      );
      ...
      logger.error({ error: error },"Error updating user");
  ...
  async delete(id: number): Promise<boolean> {
    try {
      const rows = await this.db.query(
        `UPDATE security.users SET delete_at = NOW() WHERE id = $1 AND delete_at IS NULL RETURNING id`,
        [id],
      );
      return rows.length > 0;
    } catch (error) {
      logger.error({ error: error },"Error deleting user");
